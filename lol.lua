...
-- If config is not set, set default values
...
-- Wait until the game place is loaded
repeat
    task.wait()
...
-- Check if the current place is the desired place
...
-- Function to jump to a server
function jumpToServer()
    local sfUrl = "https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=%s&limit=%s&excludeFullGames=true"
    local req = request({
        Url = string.format(sfUrl, config.placeId, config.servers.sort, config.servers.count)
    })
    ...
    if config.servers.pageDeep > 1 then
        for i = 1, config.servers.pageDeep, 1 do
            req = request({
                Url = string.format(sfUrl .. "&cursor=" .. body.nextPageCursor, config.placeId, config.servers.sort, config.servers.count),
            })
...
-- Function to collect all lootbags
...
-- Function to find the gingerbread breakable
...
if isGingerbreadExist then
    ...
    -- Find the gingerbread breakable
